...
GET_OSCAMP_V1_API = "{}/api/v1/management/files/campaigns"
List_string = List[str]


def get_file_campaigns(uri: str, user: str, password: str, campaign_id: str = None):
    get_campaigns(uri, user, password, GET_OSCAMP_V1_API, __clean_campaign, campaign_id)


def add(uri: str, user: str, password: str, name: str, file_id: int, tags: List_string, operation: str,
        timeout: int, rollout_rate: int):
    body = {
        "name": name,
        "operation": operation,
        "timeout": timeout,
